...
class Process(): #model class for processes 
	...
	def pagegenerator(self, temp): #class function to load processes into main memory
		...
		'''newly added'''
		self.pagemap={ j : i for i, j in zip(range(0, self.end-self.start+1), range(self.start, self.end+1))}
		'''newly added'''
		...
		if len(pmem)<pmem.maxsize: #if there are free page frames in primary memory
			while True:
				if temp1 not in pmem.keys():
					...
					'''newly added'''
					print(A.residentset)
					t=copy.deepcopy(A.residentset)
					res_set_list.append(t)
	...
	if len(pmem)==pmem.maxsize:
		...
		while temp2<=timeout and len(L) > i: 
			...
			if temp1 not in pmem.keys():
				...
				'''newly added'''
				for process in processlist:
					if L[i] in process.pagemap.keys():    
...
def executeprocesses(): #main method for executing the required number of processes
	...
	while True:
		...
		if (timer>timeout or Running==None) and len(Readyqueue)>0: #if timeout and there are more processes waiting...
					...
					if isswitch:
						...
						#res_set_list.append(Running.residentset)
					else: 
		...
		#print("Working set: (P{}): {}".format(Running.id, list(Running.workingset)))
		#print("Page execution sequence:", page_exec_seq)
		
		#work_set_list.append(list(Running.workingset))
		pg_seq_list.append(page_exec_seq)
		stopwatch.resume()
		'''newly added'''
	...
	print(faultcount, "PAGE FAULTS (out of {} page requests).\n".format(accesscount))
	...
	s=s+"TIME TAKEN:" +" " +str(stopwatch)+"\n"
	s=s+str(faultcount)+ " PAGE FAULTS (out of {} page requests).\n".format(accesscount) 
	#print("FINAL REGISTER/VARIABLE STATES:")
